...
async function guidatvEpgFor(name) {
  try {
    ...
    for (const v of variants) {
      ...
      if (c && c.epg && c.epg.current) {
        return {
          current: { title: c.epg.current.title, startIso: c.epg.current.start, endIso: c.epg.current.end },
...
function nameVariants(n) {
  ...
  let s = String(n || '').trim();
  push(s);
  push(s.replace(/\s*-\s*rakuten\s*tv\s*$/i, ''));
  push(s.replace(/\s*rakuten\s*tv\s*$/i, ''));
    ...
    try {
      ...
      for (const u of uni.channels) {
        ...
        if (u.epg && u.epg.current && u.epg.current.title) {
          pool.set(k, {
            current: { title: u.epg.current.title, startIso: u.epg.current.start, endIso: u.epg.current.end },
...
async function chEpg(ch) {
  ...
  if (ch.provider === 'rakuten') {
    const g = await guidatvEpgFor(ch.name);
...
module.exports = {
  ...
  getPlutoStitcherParams, dropPlutoStitcherParams, plutoQuery, chEpg,
