...
function loadPluto() {
  return cachedLoad('pluto', async () => {
    ...
    if (!r.ok) throw new Error(`pluto api HTTP ${r.status}`);
...
let plutoSession = null;

async function getPlutoStitcherParams() {
  const TTL = 25 * 60 * 1000;
  if (plutoSession && Date.now() - plutoSession.at < TTL) return plutoSession.params;
  const q = new URLSearchParams({
    appName: 'web',
    appVersion: '9.0.0',
    deviceVersion: '120.0',
    ...
    deviceType: 'web',
    clientVersion: '9.0.0',
    clientID: b64uEnc('nuvio-' + Date.now()).slice(0, 8) + '-5c1b-4a1b-b2cc-7aa2cb70e863',
    clientModelNumber: '1.0'
  });
  const r = await fetchWithTimeout('https://boot.pluto.tv/v4/start?' + q.toString(), {}, 15000);
  if (!r.ok) throw new Error(`pluto boot HTTP ${r.status}`);
  const j = await r.json();
  if (!j.stitcherParams) throw new Error('pluto boot: no stitcherParams');
  plutoSession = { params: String(j.stitcherParams), at: Date.now() };
  return plutoSession.params;
}

function plutoStreamUrl(id) {
  const q = new URLSearchParams({
    deviceId: 'channel',
    deviceModel: 'web',
    deviceVersion: '1.0',
    ...
    deviceDNT: '1',
    advertisingId: 'channel',
    embedPartner: 'rokuChannel',
    appName: 'rokuchannel',
    is_lat: '1',
    bmodel: 'bm1',
    content: 'channel',
    platform: 'web',
    tags: 'ROKU_CONTENT_TAGS',
    coppa: 'false',
    content_type: 'livefeed',
    rdid: 'channel',
    genre: 'ROKU_ADS_CONTENT_GENRE',
    content_rating: 'ROKU_ADS_CONTENT_RATING',
    studio_id: 'viacom',
    channel_id: 'channel'
  });
  return { label: 'Roku', url: `https://service-stitcher.clusters.pluto.tv/v1/stitch/embed/hls/channel/${id}/master.m3u8?${q.toString()}` };
}

function plutoDntUrl(id) {
  const q = new URLSearchParams({
    terminate: 'false',
    deviceType: 'web',
    deviceMake: 'web',
  ...
  return { label: 'DNT', url: `https://service-stitcher.clusters.pluto.tv/stitch/hls/channel/${id}/master.m3u8?${q.toString()}` };
}
...
async function plutoVariants(id) {
  const out = [];
  out.push(plutoStreamUrl(id));
  try {
    const sp = await getPlutoStitcherParams();
    const webUrl = `https://service-stitcher.clusters.pluto.tv/stitch/hls/channel/${id}/master.m3u8?${sp}`;
    out.push({ label: 'Web', url: webUrl });
    out.push({ label: 'Web App', url: webUrl, headers: PLUTO_WEB_HEADERS });
  } catch (e) { /* boot fallita: ok */ }
  out.push(plutoDntUrl(id));
  return out;
}
...
module.exports = {
  ...
  plutoStreamUrl, plutoDntUrl, plutoVariants, samsungStreamUrl, rakutenStreamUrl,
