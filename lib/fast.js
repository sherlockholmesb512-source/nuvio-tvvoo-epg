...
async function getPlutoStitcherParams() {
  ...
  if (plutoSession && Date.now() - plutoSession.at < TTL) return plutoSession;
  const q = new URLSearchParams({
    appName: 'web',
    appVersion: '9.19.0',
    deviceVersion: '120.0',
  ...
  plutoSession = { params: String(j.stitcherParams), jwt: String(j.sessionToken), at: Date.now() };
  return plutoSession;
}

async function plutoStreamUrl(id) {
  const s = await getPlutoStitcherParams();
